...
### 1. 트렌드 데이터 수집
```
python tools/youtube_fetch.py --region KR --top 10 --output .tmp/trending_{date}.json
```
...
```
(`video_id`~`transcript_available` 필드는 1·2단계 결과를 그대로 복사하고, `topic_tags`/`hook_style`/`analysis_summary`만 3단계에서 새로 채운다.)
...
- Gmail 인증 실패(토큰 만료 등)는 헤드리스 환경에서 대화형 재인증을 시도하지 않는다. 7단계는 계속 진행해 데이터 유실은 막되, 발송 실패를 명확히 보고한다 (로컬에서 수동 재인증 필요).
...
### 8. 정리
`.tmp/`는 폐기 가능한 공간이다. 7일 지난 `trending_*`/`report_*`/`transcripts/*` 파일은 정리하되, 직전 실행분은 다음 실행 전까지 보관한다.
...
| 단계 | 실패 시 |
|---|---|
| 수집 | 1회 재시도 → 계속 실패 시 발송 없이 중단, 2일 연속 실패면 사용자 보고 |
