...
```
클라우드 실행 환경의 시스템 시계는 UTC이고, 스케줄은 08:00 KST(=23:00 UTC 전날)에 실행된다. 그냥 `date +%Y-%m-%d`를 쓰면 UTC 기준 날짜가 나와서 하루 전 날짜로 잘못 찍힌다 (예: 실제 한국시간 7/11 08:00에 실행됐는데 UTC로는 아직 7/10 23:00이라 `{date}`가 "7/10"이 되어버림). 이 하루 어긋남은 표시 오류로 끝나지 않고, 2단계의 "최근 7일 반복 제외" 판정 기준일이 하루씩 밀려서 정확도가 떨어지는 원인이 된다.
...
```
- `--top 30`으로 넉넉하게 후보군을 모은다 (최종 10개는 다음 단계에서 "최근 반복 여부"를 걸러서 뽑는다). 후보가 넉넉해야 반복 제외 후에도 10개가 남는다.
...
```
python tools/rank_by_growth.py --input .tmp/candidates_{date}.json --date {date} --database-id $NOTION_DATABASE_ID --top 10 --exclude-days 7 --output .tmp/trending_{date}.json
```
- 최근 7일 이내 Notion에 이미 실린 적 있는 영상은 후보에서 완전히 제외한다 (이미 본 영상은 계속 성장 중이어도 리포트 입장에서 새로운 정보가 아니기 때문). 제외 후 남은 후보를 조회수 순으로 정렬해서 상위 10개를 뽑는다.
- 제외하고 나니 10개가 안 남는 경우(후보 30개 대부분이 최근에 이미 등장했던 드문 경우)에는, 조회수 높은 반복 영상으로 부족분을 채워서라도 10개를 유지한다 (완전히 빈 리포트보다는 낫다).
...
- **중요**: 앞으로 7일간 "최근 반복 제외" 판정(2단계)의 기준 데이터가 되므로, 이 단계가 반드시 성공해야 한다. 실패한 채로 넘어가면 그날 영상들이 제외 목록에서 빠져서 다음날 다시 등장할 수 있다 (치명적이진 않지만, 며칠 연속 실패하면 사용자에게 보고).
